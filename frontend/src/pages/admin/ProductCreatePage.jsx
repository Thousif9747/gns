...
export default function ProductCreatePage() {
  ...
  const [form, setForm] = useState({
    name: '', slug: '', description: '', base_price: '', mrp: '', category: '', unit_label: '', newImages: [],     is_active: true, is_available: true, is_featured: false,
    variants: [],
  })
  const { showToast } = useToast()

  useEffect(() => {
    get('/catalog/categories/').then(r => { if (r.ok) setCategories(extractList(r.data)) }).catch(() => showToast({ title: 'Error', message: 'Failed to load categories', type: 'error' }))
  }, [])
  ...
  function handleCropModalSave({ objectPosition, positions }) {
    ...
    setForm(prev => ({
      ...
      newImages: prev.newImages.map((img, i) =>
        i === idx ? { ...img, is_homepage_banner: true, object_position: objectPosition, card_object_position: cardPos } : img
      ),
    }))
    setCropModal(null)
  }
  ...
  async function handleSave(e) {
    ...
    payload.append('base_price', String(parseFloat(form.base_price || 0)))
    if (form.mrp) payload.append('mrp', String(parseFloat(form.mrp)))
    ...
    const res = await post('/catalog/products/', payload)
    if (res.ok) {
      // Create variants after product is successfully created
      if (form.variants.length > 0 && res.data?.slug) {
        for (const v of form.variants) {
          if (v.name && v.price) {
            const varRes = await post('/catalog/product-variants/', {
              product: res.data.id,
              name: v.name,
              sku: v.sku || '',
      ...
      showToast({ title: 'Created', message: 'Product created successfully', type: 'success' })
  ...
  return (
    <div className="ops-route ops-admin-page space-y-6" data-page="product-create">
      <PageHeader eyebrow="Catalog" title="Add product" description="Create a complete, storefront-ready product." />
                  ...
                  {form.newImages.map((image, index) => (
                    <div key={index} className="space-y-2">
                      <div className="relative overflow-hidden rounded-lg border bg-white">
                          <img src={image.preview} className="h-32 w-full object-cover" />
                        {image.is_homepage_banner ? (
                          <div className="absolute inset-x-2 bottom-2 flex items-center gap-1">
                            <button
                              type="button"
                              onClick={() => {
                                setCropModal({ key: `new-${index}`, imageUrl: image.preview, objectPosition: image.object_position, cardObjectPosition: image.card_object_position })
                              }}
                          ...
                          <button
                            type="button"
                            onClick={() => setCropModal({ key: `new-${index}`, imageUrl: image.preview, objectPosition: image.object_position, cardObjectPosition: image.card_object_position })}
        ...
        <div>
          <Card className="p-6">
            <div className="space-y-3">
              ...
              <Button type="submit" className="w-full mt-4" disabled={saving}>{saving ? 'Saving...' : 'Create Product'}</Button>
            </div>
          </Card>
        </div>
      </form>
